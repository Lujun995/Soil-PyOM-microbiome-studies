...
#      names.col <- deparse(substitute(names.col))
      data <- caicStyleArgs(data = data, phy = phy, names.col = names.col,force.root = TRUE)
    }
  }
#  binvar <- deparse(substitute(binvar))
  ...
  if (is.null(data$vcv)) {
#    vcv<-caper::VCV.array(data$phy) 
    vcv <- ape::vcv(data$phy)  #the result is almost the same, difference <1e-10 but much faster
    #root or not influences the result
  }
  else {
    vcv <- data$vcv
  }
  ...
  if (sum(is.na(ransocc))+sum(is.na(physocc))>0)
    warning("NA values in ransocc|physocc are removed.")
  ransocc <- colSums(ds.ran.cc$contrMat,na.rm = T)
  physocc <- colSums(ds.phy.cc$contrMat,na.rm = T)
  ...
  soccpval1 <- (sum(ransocc <= obssocc)+1)/(permut+1) #it's preferable
  soccpval0 <- (sum(physocc >= obssocc)+1)/(permut+1)
  dvals <- list(DEstimate = soccratio, Pval1 = soccpval1, 
                ...
                StatesTable = propStates, Permutations = list(random = ransocc, 
                                                              brownian = physocc), NodalVals = list(observed = ds.ran.cc$nodVals[, 
                                                                                                                                 1, drop = FALSE], random = ds.ran.cc$nodVals[, -1, 
                                                                                                                                                                              drop = FALSE], brownian = ds.phy.cc$nodVals[, -1, 
